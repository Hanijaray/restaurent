...
const UnavailablePopup = ({ onClose, onConfirm }) => {
    const handleConfirm = (reason) => {
        onConfirm(reason); // Pass only the reason
    };
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white flex p-8 rounded-2xl shadow-lg w-[600px] h-[350px] text-center relative">
                ...
                <div className="p-4 w-1/2">
                    <button onClick={() => handleConfirm("CHEF UNAVAILABLE")}
                        className="w-full bg-red-600 text-white py-3 rounded-lg text-lg font-semibold my-2 hover:bg-red-700">
                        CHEF UNAVAILABLE
                    </button>
                    <button onClick={() => handleConfirm("GROCERY UNAVAILABLE")}
                        className="w-full bg-red-600 text-white py-3 rounded-lg text-lg font-semibold my-2 hover:bg-red-700">
                        GROCERY UNAVAILABLE
                    </button>
                    <button onClick={() => handleConfirm("UNAVAILABLE")}
                        className="w-full bg-red-600 text-white py-3 rounded-lg text-lg font-semibold my-2 hover:bg-red-700">
                        UNAVAILABLE
                    </button>
